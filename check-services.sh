...
if [ "$BACKEND_OK" = true ] && [ "$FRONTEND_OK" = true ]; then
    ...
    echo -e "    Kiosko:      ${GREEN}http://localhost:3000/kiosk${NC}"
    echo -e "    Turnos:      ${GREEN}http://localhost:3000/turnos${NC}"
    echo -e "    Display:     ${GREEN}http://localhost:3000/display${NC}"
    echo ""
    echo -e "--- URLs de Red Local ---"
    LOCAL_IP=$(hostname -I | awk '{print $1}')
    echo -e "    Kiosko:      ${GREEN}http://$LOCAL_IP:3000/kiosk${NC}"
    echo -e "    Turnos:      ${GREEN}http://$LOCAL_IP:3000/turnos${NC}"
    echo -e "    Display:     ${GREEN}http://$LOCAL_IP:3000/display${NC}"
    echo ""
    echo -e "    ${GREEN}chromium-browser --kiosk http://$LOCAL_IP:3000/display${NC}"
    echo ""
else
    echo -e "${RED}╔════════════════════════════════════════╗${NC}"
